...
**Rule source:** [`mcp/code_execution.yaml`](https://github.com/trustabl/trustabl-rules/blob/main/mcp/code_execution.yaml)
**Rules (shipped set):** MCP-009, MCP-014
...
> the dynamic-eval threat model with the OpenAI Agents SDK pack
> ([openai_sdk/code_execution.md](https://github.com/trustabl/trustabl-rulebook/blob/main/docs/Policy/openai_sdk/code_execution.md));
...
by an untrusted document or a prompt-injected task can reach the primitive and
run whatever it constructs (LLM06), and the result returns across the trust
boundary (LLM05).
...
Remove `eval`/`exec`/`compile` and `new Function`. For arithmetic use
`ast.literal_eval`; for richer expressions validate the AST against an allow-list
before compiling; in TypeScript, parse structured input with `JSON.parse` and
dispatch on a fixed map of allowed operations. If the tool is intentionally a
code-execution surface, run the code in a separate sandboxed single-use process
(no network, read-only filesystem, CPU/wall-clock caps), never in the server
process.
