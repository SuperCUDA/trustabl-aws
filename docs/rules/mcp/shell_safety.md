...
**Rule source:** [`mcp/shell_safety.yaml`](https://github.com/trustabl/trustabl-rules/blob/main/mcp/shell_safety.yaml)
**Rules (shipped set):** MCP-010, MCP-012
...
> the process-spawn threat model with the OpenAI Agents SDK pack
> ([openai_sdk/shell_safety.md](https://github.com/trustabl/trustabl-rulebook/blob/main/docs/Policy/openai_sdk/shell_safety.md));
...
Prefer a typed library API over shelling out. If a subprocess is required, pass
argv as a list/array (never `shell=True` or `shell: true`), set a timeout, route
through an explicit command allow-list, and run the server under an OS sandbox
