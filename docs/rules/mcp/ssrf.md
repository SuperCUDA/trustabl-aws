...
**Policy ID:** `mcp_ssrf`
**Rule source:** [`mcp/ssrf.yaml`](https://github.com/trustabl/trustabl-rules/blob/main/mcp/ssrf.yaml)
**Rules (shipped set):** MCP-008, MCP-013
...
> the SSRF threat model with the OpenAI Agents SDK pack
> ([openai_sdk/ssrf.md](https://github.com/trustabl/trustabl-rulebook/blob/main/docs/Policy/openai_sdk/ssrf.md));
...
Constrain the destination to an explicit allow-list of hosts (or a single fixed
base URL with only a path parameter), reject requests to private, loopback, and
link-local IP ranges after DNS resolution, and disable redirects to addresses
outside the allow-list. An opaque ID resolved against a server-side registry —
with the URL built from the trusted entry — is the safest shape.
