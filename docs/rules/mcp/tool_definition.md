...
**Rule source:** [`mcp/tool_definition.yaml`](https://github.com/trustabl/trustabl-rules/blob/main/mcp/tool_definition.yaml)
**Rules (shipped set):** MCP-001, MCP-002, MCP-003, MCP-011
**References:** LLM06 (Excessive Agency)

> This is the AWS-plugin copy of the rationale. The canonical rulebook shares
> the structural-hygiene threat model with the OpenAI Agents SDK pack
> ([openai_sdk/tool_definition.md](https://github.com/trustabl/trustabl-rulebook/blob/main/docs/Policy/openai_sdk/tool_definition.md));
...
The structural hygiene of Model Context Protocol tool registrations — the Python
decorator forms (`@server.tool` / `@mcp.tool` / `.register_tool`, predicate
`mcp_tool` kind) and the TypeScript `@modelcontextprotocol/sdk`
...
schemas — across a transport to **whatever client and model connect to it**. The
author does not control, and often cannot see, the consuming agent. A weak
...
surface inside the server. Medium severity: degraded validation is a reliability
and minor injection-surface concern, not a direct compromise.

### MCP-003 — Ambiguous tool name (Severity: low, Confidence: 0.85, Fix type: code)

**What we detect:** a tool named from a fixed ambiguous set (`process`, `handle`,
`run`, `execute`, ...) via `name_in`.
...
- **MCP-001 / MCP-011:** give every tool a clear, single-purpose description (a
  Python docstring, or the `description` field on the TypeScript registration).
  State what the tool does and when the model should call it.
- **MCP-002:** type-annotate every handler parameter so MCP publishes a
  constrained input schema.
- **MCP-003:** rename ambiguous tools to a verb-noun that signals intent
  (`search_orders`, not `run`).
...
discovered. Non-Python/TypeScript SDK surfaces (Go, C#, PHP, Rust) are covered by
later rules outside this shipped set.
