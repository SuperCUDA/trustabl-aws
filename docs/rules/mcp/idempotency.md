...
**Rule source:** [`mcp/idempotency.yaml`](https://github.com/trustabl/trustabl-rules/blob/main/mcp/idempotency.yaml)
**Rules (shipped set):** MCP-007
**References:** LLM06 (Excessive Agency)

> This is the AWS-plugin copy of the rationale. The canonical rulebook shares
> the retry-safety threat model with the OpenAI Agents SDK pack
> ([openai_sdk/idempotency.md](https://github.com/trustabl/trustabl-rulebook/blob/main/docs/Policy/openai_sdk/idempotency.md));
...
de-duplicate server-side so a retried call is a no-op after the first success.
